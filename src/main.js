import { EvenAppBridge, CreateStartUpPageContainer, TextContainerUpgrade, TextContainerProperty, waitForEvenAppBridge, BridgeEvent, OsEventTypeList } from "@evenrealities/even_hub_sdk";
let bridge;
let socket = null;
...
const g2SimulatorText = document.getElementById('g2-text');
// HUD Elements
...
const glassesSync = document.getElementById('glasses-sync');
...
async function init() {
    ...
    try {
        ...
        window.addEventListener(BridgeEvent.EvenHubEvent, (e) => {
...
async function transcribeAudio(base64Wav) {
    try {
        updateGlasses("Processing..."); // Set to Processing state with spinner
        if (!GEMINI_API_KEY) {
            throw new Error("GEMINI_API_KEY is not configured.");
        }
        // Use Gemini 3 Flash Preview for stable and less hallucinatory STT performance
        ...
        if (!transcript || transcript === "NO_VOICE") {
            updateGlasses("Ready\n(no voice detected)");
            setTimeout(() => { if (!isRecording)
                updateGlasses("Ready"); }, 3000);
        }
        else {
            sendText(transcript);
        }
...
let glassesAnimInterval = null;
function updateGlasses(text) {
    if (loadingInterval) {
...
function startGlassesSpinner(verb) {
    let frame = 0;
    const frames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"];
...
async function connectToGateway() {
    ...
    if (!url)
        return alert("Please enter Gateway URL");
            ...
            if (data.event === "connect.challenge") {
                ...
                if (typeof p?.message === 'string') {
                    ...
                    if (Array.isArray(p.message.content)) {
                        text = p.message.content.map((c) => c.text || "").join("");
                    }
                    else {
                ...
                if (lowerText.includes("heartbeat") || lowerText === "ok" || lowerText === "connected")
                    return;
                if (text) {
...
function addHistoryItem(sender, text) {
...
function truncateForGlasses(text, maxLen = 200) {
    if (!text)
        return text;
